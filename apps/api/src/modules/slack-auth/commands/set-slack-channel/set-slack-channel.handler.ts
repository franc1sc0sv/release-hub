...
import { SetSlackChannelCommand } from './set-slack-channel.command'

@CommandHandler(SetSlackChannelCommand)
export class SetSlackChannelHandler extends BaseCommandHandler<SetSlackChannelCommand, SlackConnectionStatus> {
  constructor(
  ...
  protected async handle(
    command: SetSlackChannelCommand,
    tx: TxClient,
    ...
    const connection = await this.slackConnectionRepository.updateChannel(
      command.projectId,
      { channelId: command.channelId, channelName: command.channelName },
      tx,
    )
